...
import { FiHeart, FiPlay, FiPlus, FiMusic, FiFolder, FiCompass, FiDisc, FiHardDrive, FiTrash2 } from "react-icons/fi";
...
export default function LibraryDashboard() {
    ...
    const [activeTab, setActiveTab] = useState<'discover' | 'liked' | 'playlists' | 'local'>('discover');
        ...
        return (
            <div className="group relative bg-white/5 border border-white/5 hover:bg-white/10 rounded-2xl p-4 transition-all hover:-translate-y-1 hover:shadow-xl hover:shadow-cyan-500/10">
                <div
                    ...
                    onClick={() => {
                        store.setQueue(DISCOVERY_TRACKS);
    ...
    const TrackListItem = ({ track, onDelete }: { track: Track, onDelete?: () => void }) => {
        const isPlaying = store.activeTrack?.id === track.id && store.isPlaying;
        return (
        ...
        <div className="flex h-screen bg-transparent overflow-hidden">
            <aside className="w-64 flex-shrink-0 border-r border-white/5 bg-black/20 backdrop-blur-xl hidden md:flex flex-col p-6">
                <div className="flex items-center gap-2 mb-10 text-cyan-400">
                    <FiDisc size={28} />
                    <h1 className="text-xl font-bold tracking-tight text-white">Music<span className="text-cyan-400">Player</span></h1>
                </div>
                ...
                <nav className="space-y-2 flex-1">
                    ...
                    <SidebarItem
                        ...
                        onClick={() => { setActiveTab('local'); setSelectedPlaylistId(null); }}
                    />
                </nav>
            ...
            <main className="flex-1 overflow-y-auto p-6 pb-32 scroll-smooth">
                ...
                >
                    ...
                    {activeTab === 'local' && (
                        <div className="max-w-4xl">
                            <h2 className="text-3xl font-bold mb-6 flex items-center gap-3">
                                <span className="bg-cyan-500/20 p-3 rounded-full text-cyan-400"><FiHardDrive fill="currentColor" /></span>
                                Local Files
                            </h2>

                            {store.localTracks.length === 0 ? (
                                <div className="text-center py-20 border border-dashed border-white/10 rounded-2xl bg-white/5">
                                    <FiHardDrive size={48} className="mx-auto mb-4 opacity-20" />
                                    <p className="text-gray-400">No local files imported.</p>
                                    <p className="text-sm text-gray-500 mt-2">Use the upload button in the player bar to add songs.</p>
                                </div>
                            ) : (
                                <div className="space-y-2">
                                    {store.localTracks.map((track: Track) => (
                                        <TrackListItem
                                            key={track.id}
                                            track={track}
                                            // Pass the delete handler specifically for this view
                                            onDelete={() => {
                                                if (confirm(`Remove "${track.title}" from local files?`)) {
                                                    store.removeLocalTrack(track.id);
                                                }
                                            }}
                                        />
                                    ))}
